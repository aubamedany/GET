...
def get_radius(dist: torch.Tensor, nu: float):
    ...
    radius=torch.as_tensor(np.quantile(np.sqrt(dist.clone().data.cpu()), 1 - nu))
    if radius<0.1:
...
class CharManFitterQueryRepr1(MultiLevelAttentionCompositeFitter):
    ...
    def __init__(self, net: base_model.BaseModel,
        ...
        self.radius=torch.tensor(0, device=f'cuda:0')
...
#---------------------------------total_loss---------------------------------------------#
        if self.args.use_oc:
            ...
            loss  = (1-self.args.lamda1)*sup_loss +self.args.lamda1*reg_loss
        return loss
